...
df = pd.read_csv('/home/galkesten/transformers_project/agg_mae_spearman_kernel1.csv')
df = df.sort_values(['gradient_type', 'timestep'])
df['gradient_type'] = df['gradient_type'].mask(df['gradient_type'] == df['gradient_type'].shift(), '')
df.to_csv('grouped_table.csv', index=False)
